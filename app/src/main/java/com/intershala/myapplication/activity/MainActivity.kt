...
import com.intershala.myapplication.adapter.RecyclerFoodItemAdapter
import com.intershala.myapplication.datamodels.CartItem
import com.intershala.myapplication.datamodels.MenuItem
import com.intershala.myapplication.fragment.BottomSheetSelectedItemDialog
import com.intershala.myapplication.interfaces.MenuApi
import com.intershala.myapplication.interfaces.RequestType
import com.intershala.myapplication.services.DatabaseHandler
import com.intershala.myapplication.services.FirebaseDBService
...
class MainActivity : AppCompatActivity(),RecyclerFoodItemAdapter.OnItemClickListener, MenuApi {
    ...
    private var studentGender = "male"
...
/*        if (searchIsActive) {
    ...
    private fun loadProfile() {
        ...
        findViewById<TextView>(R.id.top_wish_name_tv).text = "Hi ${this.studentName.split(" ")[0]}"
        Handler().postDelayed({
            findViewById<TextView>(R.id.nav_header_user_name).text = this.studentName
        }, 1000)

        databaseRef.child("students")
            .child(user.uid).addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    studentGender = snapshot.child("gender").value.toString()
    ...
    private fun shareApp() {
        val message =
            "Try out this awesome App on Google Play!\n unfortunately This app is not available on Play Store so no need to open Play Store "
    ...
    private fun openUserProfileActivity() {
        ...
        intent.putExtra("gender", this.studentGender)

...
import com.intershala.myapplication.adapter.RecyclerOrderItemAdapter
...
/*    fun changeOrderTakeAwayDate(view: View) {
        val c = Calendar.getInstance()
        val date = c.get(Calendar.DATE)
        val month = c.get(Calendar.MONTH)
        val year=c.get(Calendar.DAY_OF_YEAR)
        
        val datePickerDialog = DatePickerDialog(this, this, date, month,year)
...
/*    override fun onDateSet(p0: DatePicker?, date:Int, month: Int, year: Int) {
        val date="$date:$month:$year"
        val simpleDateFormat =SimpleDateFormat("dd:mm:yyyy")
        try {
            val dateFormat=simpleDateFormat.parse(date)
            val simpleDateFormat=SimpleDateFormat("dd:mm:yyyy")
            orderTakeAwayTV.text=simpleDateFormat.format(dateFormat)
        }catch (e: Exception){}
    }*/
